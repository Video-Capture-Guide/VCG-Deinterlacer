...
REM -- Arguments (CI passes these; local double-click uses the defaults) --
REM    %1 = source .py filename         (default: current release source)
REM    %2 = version string  e.g. 1.7.6  (default: current release version)
set SOURCE=%~1
if "%SOURCE%"=="" set SOURCE=vcg_deinterlacer_v126.py
set VERSION=%~2
if "%VERSION%"=="" set VERSION=1.7.6

set OUTNAME=VCG_Deinterlacer_%VERSION%.exe

REM Numeric Windows file version (x.x.x.x). Real releases like 1.7.6 qualify;
REM CI dev builds (e.g. dev-abc1234) fall back to 0.0.0.0.
set FILEVER=0.0.0.0
echo(%VERSION%|findstr /r "^[0-9][0-9]*\.[0-9][0-9]*\.[0-9][0-9]*$" >nul && set FILEVER=%VERSION%.0

echo.
echo ============================================================
echo  VCG Deinterlacer %VERSION% - Build Script
echo ============================================================
echo.
...
if not exist "dist\%OUTNAME%" (
    echo.
    echo ============================================================
    echo  BUILD FAILED  ^(dist\%OUTNAME% not produced^)
    echo ============================================================
    echo.
    echo Common causes:
...
echo.
echo Next steps:
echo   1. Test the EXE:  dist\%OUTNAME%
echo   2. Package as ZIP (%VERSION% portable -- no installer needed):
echo      - Create a folder:  VCG_Deinterlacer_%VERSION%\
echo      - Copy into it:     dist\%OUTNAME%
echo      -                   logo.png
echo      -                   README.txt  (rename from README.md)
echo      -                   LICENSE.txt
echo      - Zip the folder:   VCG_Deinterlacer_%VERSION%.zip
