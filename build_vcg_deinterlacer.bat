...
echo.
echo ============================================================
echo  VCG Deinterlacer 1.0.10 - Build Script
echo ============================================================
echo.
...
set SOURCE=vcg_deinterlacer_v110.py
if not exist "%SOURCE%" (
...
python -m nuitka ^
    ...
    --output-filename=VCG_Deinterlacer_1.0.10.exe ^
    --output-dir=dist ^
    %SOURCE%
...
if not exist "dist\VCG_Deinterlacer_1.0.10.exe" (
    echo.
    echo ============================================================
    echo  BUILD FAILED  ^(dist\VCG_Deinterlacer_1.0.10.exe not produced^)
    echo ============================================================
    echo.
    echo Common causes:
...
echo ============================================================
echo.
echo Output:  dist\VCG_Deinterlacer_1.0.10.exe
echo.
...
echo.
echo Next steps:
echo   1. Test the EXE:  dist\VCG_Deinterlacer_1.0.10.exe
echo   2. Package as ZIP (1.0.4 portable -- no installer needed):
echo      - Create a folder:  VCG_Deinterlacer_1.0.10\
echo      - Copy into it:     dist\VCG_Deinterlacer_1.0.10.exe
echo      -                   logo.png
echo      -                   README.txt  (rename from README.md)
echo      -                   LICENSE.txt
echo      - Zip the folder:   VCG_Deinterlacer_1.0.10.zip
